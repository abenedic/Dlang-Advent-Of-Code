...
token get_token(string s)
{
    ...
    {
        output.t = type.lshift;
    }
    else if (s == "RSHIFT")
    {
        output.t = type.rshift;
    }
    else if (s == "OR")
    {
        output.t = type.or;
...
void interpret_line(string s, ref operation[string] o)
{
    token[] t = lex(s);
    ...
    else if (t[0].t == type.integer && t[1].t == type.arrow && t[2].t == type.id)
    {
        operation op;
        op.t = type.assign;
        op.args[0] = t[0];
...
operation[string] get_out_wires(string input)
{
