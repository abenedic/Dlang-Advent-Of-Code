...
token get_token(string s)
{
    ...
    if (s[0] == '-' && s[1] == '>')
    {
        output.t = token.type.arrow;
    }
    else if (s == "NOT")
    {
        output.t = token.type.not;
    }
    else if (s == "AND")
    {
        output.t = token.type.and;
    }
    else if (s == "LSHIFT")
    {
        output.t = token.type.lshift;
    }
    else if (s == "RSHIFT")
    {
        output.t = token.type.rshift;
    }
    else if (s == "OR")
    {
        output.t = token.type.or;
    }
    else if (s[0].isAlpha())
    {
        output.t = token.type.identifier;
    }
    else if (s[0].isDigit())
    {
        output.t = token.type.integer;
    }
    else
    {
        output.t = token.type.none;
    }
...
void interpret_line(string s, ref ushort[string] o)
{
    ...
    else if (t[0].t == token.type.identifier && (t[2].t == token.type.identifier || t[2].t == token.type.integer) && t[4].t == token.type.identifier)
    {
        ...
        switch(t[1].t){
            ...
            && t[2].t == token.type.identifier)
    {

        o[t[2].value] = to!ushort(t[0].value);
        return;
    }
    assert(0);
    return;
...
ushort[string] get_out_wires(string input)
{
    ushort[string] output;
    foreach (string s; input.splitLines())
    {
        writeln(s);
        interpret_line(s, output);
    }
    return output;
}
...
NOT y -> i`;
    ...
    assert(array["f"] == 492);
    assert(array["g"] == 114);
    assert(array["h"] == 65412);
    assert(array["i"] == 65079);
    assert(array["x"] == 123);
    assert(array["y"] == 456);
}
